...
func TestPrevID_Decrements(t *testing.T) {
	got, ok := prevID("100")
	if !ok || got != "99" {
		t.Fatalf("prevID(100) = %q,%v want 99,true", got, ok)
	}
}

func TestPrevID_Boundary(t *testing.T) {
	for _, in := range []string{"0", "", "notanumber", "-5", "12.3"} {
		if got, ok := prevID(in); ok {
			t.Fatalf("prevID(%q) = %q,true want _,false", in, got)
		}
	}
}

func TestPrevID_LargeSnowflake(t *testing.T) {
	// Must use uint64; int overflow or float rounding would corrupt this.
	got, ok := prevID("1580661436132757506")
	if !ok || got != "1580661436132757505" {
		t.Fatalf("prevID large = %q,%v want 1580661436132757505,true", got, ok)
	}
}

func TestIDLess_Numeric(t *testing.T) {
	// String compare would call "9" > "100"; numeric must say 9 < 100.
	if !idLess("9", "100") {
		t.Fatal("idLess(9,100) should be true numerically")
	}
	if idLess("100", "9") {
		t.Fatal("idLess(100,9) should be false")
	}
	if idLess("100", "100") {
		t.Fatal("idLess equal should be false")
	}
	// Unparseable sorts last: a valid ID is always "less".
	if !idLess("5", "x") || idLess("x", "5") {
		t.Fatal("unparseable id should sort last")
	}
}

func TestBuildBackfillQuery_Base(t *testing.T) {
	if q := buildBackfillQuery("bob", "99", backfillOpts{}); q != "from:bob max_id:99" {
		t.Fatalf("base query = %q", q)
	}
}

func TestBuildBackfillQuery_FirstWindow(t *testing.T) {
	if q := buildBackfillQuery("bob", "", backfillOpts{}); q != "from:bob" {
		t.Fatalf("first-window query = %q want no max_id", q)
	}
}

func TestBuildBackfillQuery_Flags(t *testing.T) {
	q := buildBackfillQuery("bob", "99", backfillOpts{retweets: true, mediaOnly: true})
	if !strings.Contains(q, "filter:media") {
		t.Fatalf("mediaOnly missing filter:media: %q", q)
	}
	if !strings.Contains(q, "include:nativeretweets") {
		t.Fatalf("retweets missing include:nativeretweets: %q", q)
	}

	q = buildBackfillQuery("bob", "99", backfillOpts{})
	if strings.Contains(q, "filter:media") || strings.Contains(q, "include:nativeretweets") {
		t.Fatalf("no-flag query should carry neither operator: %q", q)
	}
}

func TestApplyWindow_Dedup(t *testing.T) {
	seen := map[string]bool{"10": true, "20": true}
	batch := []*twitterscraper.Tweet{sampleTweet("10", "a"), sampleTweet("20", "b")}
	newTweets, _, added := applyWindow(seen, "10", batch)
	if added != 0 || len(newTweets) != 0 {
		t.Fatalf("fully-overlapping window: added=%d new=%d want 0,0", added, len(newTweets))
	}
}

func TestApplyWindow_TracksLowestNumeric(t *testing.T) {
	seen := map[string]bool{}
	// Mixed order + different lengths: string min would be "1000", numeric is "900".
	batch := []*twitterscraper.Tweet{sampleTweet("1000", "a"), sampleTweet("900", "b"), sampleTweet("1500", "c")}
	_, newOldest, added := applyWindow(seen, "2000", batch)
	if added != 3 {
		t.Fatalf("added = %d want 3", added)
	}
	if newOldest != "900" {
		t.Fatalf("newOldest = %q want 900 (numeric lowest)", newOldest)
	}
}

func TestApplyWindow_PartialOverlap(t *testing.T) {
	seen := map[string]bool{"20": true}
	batch := []*twitterscraper.Tweet{sampleTweet("30", "a"), sampleTweet("20", "b"), sampleTweet("10", "c")}
	newTweets, newOldest, added := applyWindow(seen, "30", batch)
	if added != 2 || len(newTweets) != 2 {
		t.Fatalf("partial overlap: added=%d new=%d want 2,2", added, len(newTweets))
	}
	if !seen["10"] || !seen["30"] {
		t.Fatal("new ids should be marked seen")
	}
	if newOldest != "10" {
		t.Fatalf("newOldest = %q want 10", newOldest)
	}
}
